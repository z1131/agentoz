...
public class ConfigProtoConverter {
    ...
    /**
     * 创建仅模型配置（不含指令，避免重复发送系统提示词）
     *
     * <h3>🎯 用途</h3>
     * <p>当有历史记录时，发送仅包含模型信息的配置，避免codex使用默认逻辑。</p>
     *
     * <h3>📋 包含的字段</h3>
     * <ul>
     *   <li>model - 模型名称（让codex知道这不是默认配置）</li>
     *   <li>model_provider - 模型提供商</li>
     *   <li>provider_info - 提供商详细信息</li>
     *   <li>approval_policy - 审批策略</li>
     *   <li>sandbox_policy - 沙箱策略</li>
     *   <li>cwdr - 工作目录</li>
     *   <li>mcp_servers - MCP服务器配置</li>
     * </ul>
     *
     * <h3>❌ 不包含的字段</h3>
     * <ul>
     *   <li>base_instructions - 已在history_rollout中，避免重复</li>
     *   <li>developer_instructions - 已在history_rollout中，避免重复</li>
     * </ul>
     *
     * <h3>💡 为什么不能发送空配置？</h3>
     * <p>发送空配置（getDefaultInstance()）会导致codex触发默认逻辑：</p>
     * <ul>
     *   <li>注入"codex cli"默认人设</li>
     *   <li>校验是否是默认模型</li>
     *   <li>使用默认的baseInstructions</li>
     * </ul>
     *
    ...
    public static SessionConfig toModelOnlyConfig(AgentConfigEntity entity) {
        ...
        // 2. 指令配置（清除 - 已在history_rollout中，避免重复）
        ...
        if (entity.getMcpConfigJson() != null && !entity.getMcpConfigJson().isEmpty()) {
            try {
                parseMcpServers(entity.getMcpConfigJson(), builder);
                log.debug("解析 MCP 配置成功（仅模型模式）: length={}", entity.getMcpConfigJson().length());
            } catch (Exception e) {
                log.error("解析 MCP 配置失败: {}", e.getMessage(), e);
            }
        }
